...
export default function AllTechniciens() {
  ...
  useEffect(() => {
    api.get('/techniciens').then(r => {
      const p = r.data?.data ?? [];
      setTechniciens(Array.isArray(p) ? p : (Array.isArray(p?.data) ? p.data : []));
    }).catch(() => {}).finally(() => setLoading(false));
  }, []);

  const filtered = techniciens.filter(t =>
    !search || `${t.prenom} ${t.nom} ${t.specialite || ''} ${t.adresse || ''}`.toLowerCase().includes(search.toLowerCase())
  );
        ...
        <div className="max-w-[1200px] mx-auto">
          <div className="relative max-w-md mb-8">
            ...
            <input
              ...
              type="text" placeholder={t('home.search') || 'Rechercher un technicien...'} value={search}
